      ...
      <div class="templatemo-sidebar">
        ...
        <nav class="templatemo-left-nav">
          <ul>
			...
			<li><a href="bookmanage1?type=bookmy&r_id=${READER.r_id }" class="active"><i class="fa fa-sliders fa-fw"></i>我的图书</a></li>
          ...
          <div class="row">
            <nav class="templatemo-top-nav col-lg-12 col-md-12">
              <ul class="text-uppercase">
                <li><a href="" class="active">我的书籍</a></li>
              </ul>
            </nav>
          </div>
        </div>
        <div class="templatemo-content-container">
        <form action="searchbook1?type=mybook&r_id=${READER.r_id }" method="post" class="templatemo-search-form" role="search">
		          <div class="input-group">
		              ...
		              <select name="select" id="demoSelect" ng-model="binding.value"  style="width:100%;height:26px;background-color:#39ADB4;"  ng-options="key for key in binding.landarr">
		                	<option value="tb_borrow.b_ID">书籍序号</option>
                ...
                <tbody>
                  <c:forEach var="item" items="${sessionScope.BookAppo }">
						<tr>
							...
							<td>${item.b_state }</td>
							<c:if test="${item.b_state == '已借阅'}">
								<td><a href="mybookbackTime?b_ID=${item.b_ID }&r_id=${READER.r_id }" class="templatemo-link">续约</a></td>
							</c:if>
						</tr>
				 </c:forEach>
